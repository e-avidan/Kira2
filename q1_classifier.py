...
class SoftmaxModel(Model):
    ...
    """
        ### YOUR CODE HERE
        # Create a variable.
        with tf.name_scope("lin-mul-soft") as scope:
            x = input_data
            W = tf.Variable(
                initial_value=tf.zeros(dtype=tf.float32, shape=(Config.n_features, Config.n_classes), name="weights"))
            b = tf.Variable(initial_value=tf.zeros(dtype=tf.float32, shape=(Config.n_classes), name="bias"))
            out = softmax(tf.matmul(x, W, name=scope) + b)
        ### END YOUR CODE
        return out
